...
# Starting with a halfway point between 0 and 1 of accumulated weight
accumulated_weight = 0.5


# Manually setting the ROI for grabbing the hand.
# We can change this region of interest to top right top left or anywhere we want
#For now it is set to top right
roi_top = 20
roi_bottom = 300
roi_right = 300
roi_left = 600
...
def calc_accum_avg(frame, accumulated_weight):
    '''
    Given a frame and a previous accumulated weight,
    computed the weighted average of the image passed in.
    '''
...
def segment(frame, threshold=25):
    ...
    # If length of contours list is 0, then we dont grab any contours
    if len(contours) == 0:
        return None
    else:
...
def count_fingers(thresholded, hand_segment):
    ...
    # We'll grab those points by using argmin and argmax.

    # Finding the top, bottom, left , and right ie the topmmost , bottommost, leftmost and rightmost point
    ...
    # The center of the hand should be half way between the top and bottom and halfway between left and right
    cX = (left[0] + right[0]) // 2
    cY = (top[1] + bottom[1]) // 2
    ...
    # Then we'll grab the largest distance
